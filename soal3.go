...
func urutkanKesulitan(dp *daftarProyek) {
	var i, j, minIdx int
	var temp proyek

	for i = 0; i < dp.tersimpan-1; i++ {
		minIdx = i
		for j = i + 1; j < dp.tersimpan; j++ {
...
func urutkanTanggal(dp *daftarProyek) {
	var i, j int
	var temp proyek

	for i = 1; i < dp.tersimpan; i++ {
		temp = dp.data[i]
		j = i - 1
...
func tampilkanStatistikKategori(dp daftarProyek) {
	var statistik [NMAX]statistikKategori
	var jumlahKategori, i, j int
	var ditemukan bool

	for i = 0; i < dp.tersimpan; i++ {
		ditemukan = false
		for j = 0; j < jumlahKategori; j++ {
			if dp.data[i].kategori == statistik[j].kategori {
				statistik[j].jumlah++
				ditemukan = true
			}
		}
		if !ditemukan {
			statistik[jumlahKategori].kategori = dp.data[i].kategori
			statistik[jumlahKategori].jumlah = 1
			jumlahKategori++
		}
	}

	fmt.Println("Statistik jumlah proyek per kategori:")
	for i = 0; i < jumlahKategori; i++ {
		fmt.Println(statistik[i].kategori, ":", statistik[i].jumlah)
	}
}
...
func hapusProyek(dp *daftarProyek, nama string) {
	...
	if index == -1 {
		...
		for i = index; i < dp.tersimpan-1; i++ {
